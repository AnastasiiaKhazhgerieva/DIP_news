...
1. │ SCRAPE NEWS SITES                                    │
   ...
   └──────────────────────┬───────────────────────────────┘
              ...
              ved.json, rbc.json, agro.json, ria.json, autostat.json
                          ...
                          v
   ┌──────────────────────────────────────────────────────┐
2. │ SUMMARISE ARTICLE BODIES                             │
   │ summarize_feeds()                                    │
   │                                                      │
   │ Action: for each item in every feed JSON under     │
   │ `1 news_jsons`, fetch the article HTML (via PROXY),  │
   │ extract up to 3 paragraphs / 1500 chars, call an   │
   │ LLM in batches of 8 and attach a short `summary`   │
   │ to each item. Cache URL→summary in                  │
   │ `_summary_cache.json` (reset every Saturday).       │
   └──────────────────────┬───────────────────────────────┘
                          v  same files, now with summary
              [ 1 news_jsons ]  (+ _summary_cache.json)
                          │
                          │  loop:  section* ∈ { world, rus, prices } 
                          v  *subset of the scraped sources, it produces its own weekly list / report. 
   ┌──────────────────────────────────────────────────────┐
3. │ FILTER & MERGE INTO WEEKLY LIST                      │
   │ create_news_lists(section)                           │
   │                                                      │
   │ Action: for each source JSON listed in               │
   │ section_to_files[section], send title + summary to │
   ...
   │ and re-attach published_date + summary from feed.  │
   │                                                      │
   ...
   └──────────────────────┬───────────────────────────────┘
        ...
        [ 2 4 new_lists_json ] <──── read back on non-Saturday days
                          ...
                          v
   ┌──────────────────────────────────────────────────────┐
4. │ RE-RANK BY IMPORTANCE & KEEP TOP-40                  │
   │ prioritise(section)                                  │
   │                                                      │
   │ Action: send the full weekly list (title, summary,   │
   │ published_date) to an LLM, which assigns each item   │
   │ a 0-10 importance score. Sort desc, save graded    │
   │ debug copy, cut to 40; keep summary in output.       │
   │                                                      │
...
[ 3 news_lists_json_grade ]                  [ 2 4 new_lists_json ]
                                                      ...
                                                      v
   ┌──────────────────────────────────────────────────────┐
5. │ RENDER NUMBERED .TXT & POST LINK TO TELEGRAM         │
...
═══════════════ Thursday only  (datetime.today().weekday() == 3) ═══════════════
                  ...
                  [ 2 4 new_lists_json/<section>.json ]
                          ...
                          v
   ┌──────────────────────────────────────────────────────┐
6. │ GROUP TOP-40 INTO 4 WEEKLY THEMES                    │
   ...
   └──────────────────────┬───────────────────────────────┘
                    ...
                    [ 6 news_top ]
                          ...
                          v
   ┌──────────────────────────────────────────────────────┐
7. │ DOWNLOAD & CLEAN ARTICLE BODIES                      │
   ...
   └──────────────────────┬───────────────────────────────┘
                  ...
                  [ 7 news_top_texts ]
                          ...
                          v
   ┌──────────────────────────────────────────────────────┐
8. │ FINAL BULLET REPORT & POST TO TELEGRAM               │
...
2. **Summarise** -- `summarize_feeds()`
  - **Input:** every JSON in `**1 news_jsons`** from step 1.
  - **What it does:** fetches each article body (up to 3 paragraphs / 1500 chars), calls the LLM in batches of 8, writes a `summary` field back into the same feed files. URL→summary cache in `_summary_cache.json` (cleared every Saturday).
  - **Output:** enriched feed JSONs in `**1 news_jsons`** with `{title, url, published_date, summary}`.
3. **Filter & merge** -- `create_news_lists(section)` for `section in {"world", "rus", "prices"}`
  - **Input:** enriched source JSONs from `**1 news_jsons`**; plus the existing weekly `**2 4 new_lists_json/<section>.json`** (non-Saturdays only); plus `**0_prompts/lists_<section>.txt**`.
  - **What it does:** sends each source JSON (with title + summary) to the LLM with the section prompt, keeps only relevant items, deduplicates by URL, re-attaches `published_date` and `summary` from the raw feed.
  - **Output:** updated `**2 4 new_lists_json/<section>.json`** (cumulative weekly list with summaries).
4. **Re-rank, keep top-40** -- `prioritise(section)`
  - **Input:** `**2 4 new_lists_json/<section>.json`** + `**0_prompts/prioritise_<section>.txt`**.
  - **What it does:** LLM grades each item 0--10 using title + summary; sorted descending; trimmed to 40; `summary` preserved in output.
  - **Output:**
    - debug copy with grades to `**3 news_lists_json_grade/<section>.json`**;
    - trimmed `{title, url, published_date, summary}` overwrites `**2 4 new_lists_json/<section>.json`**.
5. **Render numbered list + notify** -- `design_wo_llm(section)` (fallback `design(section)`)
...
6. **Pick top themes (Thursday only)** -- `choose_top_urls(section)`
...
7. **Download article bodies (Thursday only)** -- `read_top_urls(section)`
...
8. **Final bullets + notify (Thursday only)** -- `create_bullets(section)`
...
| `1 news_jsons`            | scrapers (`fetch_`*); `summarize_feeds` (adds `summary`; writes `_summary_cache.json`)                     |
...
python dip_news.py --stage summarize
python dip_news.py --stage prioritise

# chain several stages, in the order you list them
python dip_news.py --stage scrape summarize lists prioritise design
...
| `summarize`  | step 2 — `summarize_feeds` (fetch bodies, LLM summaries, cache in `_summary_cache.json`)         |
| `lists`      | step 3 — `create_news_lists` for `world`, `rus`, `prices`                                        |
| `prioritise` | step 4 — `prioritise` for all sections (re-rank + keep top-40)                                   |
| `design`     | step 5 — `design_wo_llm` (fallback `design`) + `telegram_lists()`                                |
| `top`        | step 6 — `choose_top_urls` (Thursday-only by default)                                            |
| `read_top`   | step 7 — `read_top_urls` (Thursday-only by default)                                              |
| `bullets`    | step 8 — `create_bullets` + `telegram_bullets()` (Thursday-only by default)                      |
