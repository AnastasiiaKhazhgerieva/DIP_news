...
import io
try: # google colab не запускается, когда раним через workflow, он там естьт по умолчанию, поэтому имени в PyPL такого нет
...
if not sa_json:
    raise RuntimeError("Сервисный аккаунт не найден в GCP_SA_KEY")

creds_info = json.loads(sa_json)  # превращаем строку в dict
...
def save_json_to_drive_root(file_name: str, data_obj):

    # 1) Преобразуем Python‐объект в JSON‐строку
    json_str = json.dumps(data_obj, ensure_ascii=False, indent=2)
    json_bytes = json_str.encode("utf-8")

    # 2) Проверим, есть ли уже файл с таким именем в root, чтобы обновить, а не создавать дубликат
    #    Для этого пробуем найти его через Drive API:
    ...
    if existing_file_id:
        ...
        # 4b) Если файла нет — создадим новый в корне ("root")
        file_metadata = {
            "name": file_name,
            "parents": ["root"],
...
def save_text_to_drive_root(file_name: str, text: str):
    """
    Сохраняет текст в файл file_name (например, "output.txt") в корне Google Drive.
    Если файл с таким именем уже существует, он будет перезаписан.
    """
    # 1) Преобразуем текст в байты UTF-8
    text_bytes = text.encode("utf-8")

    # 2) Проверим, есть ли уже файл с таким именем в root
    existing_file_id = None
    try:
...
def create_news_lists(section):
    ...
    save_text_to_drive_root(file_name, all_text)
...
def create_bullets(section):
    ...
    save_text_to_drive_root(file_name, response.text)
